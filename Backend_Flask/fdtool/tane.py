...
def computeCplus(
        x):  # this computes the Cplus from the first definition in section 3.2.2 of TANE paper. output should be a list of single attributes
    global listofcolumns
    listofcols = listofcolumns[:]
...
if len(sys.argv) > 1:
    ...
    file.write('Table : ' + str(infile.rsplit('.')[0]) + '\n')
...
file.write('Columns : ' + str(listofcolumns).replace('[', '').replace(']', '').replace("'", '') + '\n\n')
file.write('Functional Dependencies: \n')
String = ''
for fd in finallistofFDs:
    lhs = ', '.join(fd[0])
    rhs = fd[1]
    file.write('{' + lhs + '} -> {' + rhs + '}\n')
