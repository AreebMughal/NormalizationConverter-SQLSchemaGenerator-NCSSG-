...
# lowercase = u"ßäöüçáéíóúàèìòùãẽĩõũâêîôûëï";
# uppercase = u"ÄÖÜÇÁÉÍÓÚÀÈÌÒÙÃẼĨÕŨÂÊÎÔÛËÏ"
from numpy.core.defchararray import join

lowercase = string.ascii_lowercase
uppercase = string.ascii_uppercase
letters = string.ascii_lowercase + uppercase;
...
def abh2str(li, re):
    ...
    return join(li, attrsep) + '->' + join(re, attrsep);
        ...
        if len(newre) == 0:
            ...
            if len(li) > 1:
                ...
                if lired != li:
                    abh = redabh;
                    lired = frozenset(
                        lired);  # left sides need to be unchangeable frozensets in order to serve as dictionary keys
                    if lired in abh:
