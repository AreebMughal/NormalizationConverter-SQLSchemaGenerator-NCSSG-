...
const SqlSchemaGenerator = () => {
  ...
  useEffect(() => {
    ...
    axios
      .post("http://127.0.0.1:5000/getSqlSchemaData", {
        inputBoxes,
      ...
      if (relIndex !== relationIndex)
        rel.attributes.map((attr, attrIndex) => {
          if (
            ...
            attr.value ===
              relationList[relationIndex].attributes[attributeIndex].value &&
            property !== "index"
          ) {
  ...
  function isDataValid() {
    return (
      ...
      relationList[0].attributes !== undefined
    );
  }
      ...
      return (
        <div className="my-form mt-2 pt-1" key={index}>
          <div className="relation_type text-center">
            {obj.relationName[1] !== undefined
              ? obj.relationName[1].toUpperCase()
              : ""}
          </div>
          <FloatingLabel
          ...
          >
            <Form.Control
              className="input_rel_name floating-fields"
              type="text"
  ...
  return (
    <div className="mt-4">
      <SqlPageHeader onChange={normalFormChangeHandler} />
